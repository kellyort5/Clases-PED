...
class ListaDoble{
    ...
    public:
        ...
        void deleteItem(T);
        bool empty(void);
        bool searchItemByTitle(string);
        bool searchItemByYear(int);
        bool searchItemExact(T);
...
bool ListaDoble::searchItemByTitle(string titulo){
    if(!pInicio) // Lista vacía.
        return false;
    else{ // Si la lista no está vacía, entonces:
        nodo *saltarin = pInicio;
        while(saltarin && (saltarin->dato).titulo.compare(titulo)!=0) // Saltar.
            saltarin = saltarin->sig; // Luego de saltar hay varias posibilidades:
        if(!saltarin) // a) saltarín se salió (el dato no está en la lista).
            return false;
        else
...
bool ListaDoble::searchItemByYear(int anio){
    if(!pInicio) // Lista vacía.
        return false;
    else{ // Si la lista no está vacía, entonces:
        nodo *saltarin = pInicio;
        while(saltarin && (saltarin->dato).anio != anio) // Saltar.
            saltarin = saltarin->sig; // Luego de saltar hay varias posibilidades:
        if(!saltarin) // a) saltarín se salió (el dato no está en la lista).
            return false;
        else
...
bool ListaDoble::searchItemExact(T datoB){
    if(!pInicio) // Lista vacía.
        return false;
    else{ // Si la lista no está vacía, entonces:
        nodo *saltarin = pInicio;
        while(saltarin && 
              (  (saltarin->dato).titulo.compare(datoB.titulo)!=0
                || (saltarin->dato).anio != datoB.anio)) // Saltar.
            saltarin = saltarin->sig; // Luego de saltar hay varias posibilidades:
        if(!saltarin) // a) saltarín se salió (el dato no está en la lista).
            return false;
        else
...
void ListaDoble::insertAfter(T dato, T datoRef){
    ...
    if(!pInicio){
        ...
        saltarin = pInicio;
        while(saltarin->sig != NULL && 
                ((saltarin->dato).titulo.compare(datoRef.titulo)!=0
                || (saltarin->dato).anio != datoRef.anio)){
            saltarin = saltarin->sig;
        }
